...
- Folder discovery has one independent scanner, processing at most one directory per scheduler tick (every 500 ms). It continues while all file-transfer slots are busy; a slow listing does not block file completion checks or new file transfers. Global and group pauses stop new scans, and an already-started scan finishes. Discovery retains one directory listing at a time and stores discovered entries in SQLite. The total becomes known once all folders are discovered and file sizes are available; listing failures remain visible for review.
...
- Queue entries and pauses persist in SQLite. Completed-file history remains rclone's bounded history. If rclone restarts, a job expires, or a submission outcome is uncertain, affected entries require review instead of automatically replaying a possibly completed move. Check destination and active jobs, then use **Retry** in the failed entry's Actions column if needed. Retry returns files or folders to the same queue group, clears the old error/job reference, and preserves global/group pauses. A recorded job still running in the current rclone instance blocks retry. If paused, use Resume when ready.
